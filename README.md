...
- **Most Good License Vault:** `/dashboard/licenses` provides a protected company license/certification register, with 30-, 60-, or 90-day in-app renewal alerts displayed in the dashboard and included in the Excel/PDF exports. This MVP deliberately reads validated sample records from `data/dashboard.json`: Vercel serverless files cannot safely persist uploads or scheduled jobs. Before accepting actual certificate files or promising email reminders, replace the `getDashboardData()` JSON boundary with durable, tenant-scoped storage and connect a scheduled notification service. The current UI does not claim otherwise.
